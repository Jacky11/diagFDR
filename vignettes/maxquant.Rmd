...
We start with a simulated dataset that resembles the `dfdr_tbl` returned by
...
diag <- dfdr_run_all(
  xs = list(MaxQuant_PSM = x_mq),
  alpha_main = 0.01,
  compute_pseudo_pvalues = TRUE,   # <-- Automatically generates pseudo p-values from scores
...
When compute_pseudo_pvalues = TRUE with method = "decoy_ecdf":
    ...
    P-value histogram: Should show enrichment near 0 (alternative) and flatness in the tail (null)
